...
export type GanttMilestonesProps = {
  milestones: Milestone[];
  className?: string;
};

export const GanttMilestones: FC<GanttMilestonesProps> = ({
  milestones,
  className,
}) => {
  const gantt = useContext(GanttContext);
      ...
      {milestones.map((milestone) => {
        ...
        return (
          ...
          >
            <div
              className={cn(
                "group pointer-events-auto sticky top-0 flex select-auto flex-col flex-nowrap items-center justify-center whitespace-nowrap rounded-md px-2 py-1 text-xs font-medium",
                statusColor,
                className,
              )}
            ...
            >
              ...
              <span className="max-h-[0] overflow-hidden opacity-90 transition-all group-hover:max-h-[4rem]">
                {formatDate(date, "MMM dd, yyyy")}
...
// Dependency arrow connection type
export type DependencyConnection = {
  id: string;
  sourceTaskId: string;
  targetTaskId: string;
  relationType: "blocks" | "depends";
};

export type GanttDependencyArrowsProps = {
...
export const GanttDependencyArrows: FC<GanttDependencyArrowsProps> = ({
  ...
  // Create a map of task IDs to their DOM elements for quick lookup
  const taskPositions = useMemo(() => {
    ...
    tasks.forEach((task) => {
      const element = document.querySelector(
        `[data-gantt-feature-id="${task.id}"]`,
      ) as HTMLElement;
      if (element && gantt.ref?.current) {
        const containerRect = gantt.ref.current.getBoundingClientRect();
        const taskRect = element.getBoundingClientRect();
        positions.set(task.id, {
          x: taskRect.left - containerRect.left + scrollX,
          y: taskRect.top - containerRect.top,
    ...
    return positions;
  }, [tasks, scrollX, gantt.ref]);

  // Force re-render on scroll and resize
  useEffect(() => {
    ...
    gantt.ref?.current?.addEventListener("scroll", handleUpdate);

    return () => {
      window.removeEventListener("scroll", handleUpdate);
      window.removeEventListener("resize", handleUpdate);
      gantt.ref?.current?.removeEventListener("scroll", handleUpdate);
    };
  }, [gantt.ref]);
  ...
  const arrows = useMemo(() => {
    return dependencies
      .map((dep) => {
        const sourcePos = taskPositions.get(dep.sourceTaskId);
        const targetPos = taskPositions.get(dep.targetTaskId);

        if (!sourcePos || !targetPos) return null;

        // Arrow goes from right edge of source to left edge of target
  ...
  return (
    <svg
      ref={svgRef}
      className={cn(
        "pointer-events-none absolute top-0 left-0 z-10 h-full w-full overflow-visible",
        className,
      )}
      style={{
        minWidth: "100%",
    ...
    >
      ...
      {arrows.map((arrow) => (
        <g key={arrow!.id}>
          <path
            d={arrow!.path}
            stroke={arrow!.type === "blocks" ? "#ef4444" : "#3b82f6"}
            strokeWidth="2"
            fill="none"
